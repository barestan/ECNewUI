<template>
    <div class="e-list">
        <div class="e-list-content"  id="e-view" v-on:scroll="scroll" :class="{loading: loading}">
            <template v-for="(item, index) in items">
                <ErrorListItem :class="{gray: index % 2 === 0 }" v-bind:item="item.error" v-bind:log="item.log"
                                 v-bind:id="item.id" v-bind:key="item.id"></ErrorListItem>
            </template>
        </div>
        <div class="total-count">Loaded <span>{{items.length}}</span> of <span>{{totalCount}}</span> errors</div>
    </div>
        ...
        mounted: function(){
            axios.get( (window.$elmah_root || '/elmah') +'/api/errors?p=0&s=50')
                .then(response => {
                    ...
                    this.$parent.selected = this.items[0];
                })
                .catch(error => {
                    console.log(error)
        ...
        methods:{
            ...
            scroll() {
                ...
                this.root.onscroll = () => {
                    ...
                    let bottomOfWindow = this.root.scrollTop + this.root.clientHeight === this.root.scrollHeight;

                    if (bottomOfWindow) {
                        this.loading = true;
                        // eslint-disable-next-line no-undef
                        axios.get((window.$elmah_root || '/elmah') +'/api/errors?i=' + this.errorIndex + '&s=50')
                            .then(response => {
